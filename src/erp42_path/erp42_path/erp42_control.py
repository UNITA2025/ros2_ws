...
class PurePursuit(Node):
    def __init__(self):
        ...
        self.declare_parameter('control_dt', 0.01)           # timer 주기 [s]

        # 차량/조향
        self.declare_parameter('wheelbase', 1.10)
        self.declare_parameter('max_steer_deg', 20.0)           # ±조향 한계
        self.declare_parameter('max_steer_rate_deg_s', 150.0)   # ±조향 속도 한계

        # lookahead (속도 스케일)
        self.declare_parameter('lfd_gain', 0.2)  # lfd = gain * v(m/s)
        self.declare_parameter('lfd_min', 3.0)
        ...
        # 속도 커맨드 간단 로직
        self.declare_parameter('speed_cmd_straight', 60)
        self.declare_parameter('speed_cmd_turn', 30)
        ...
        # self.create_subscription(String, '/controller_mode', self.controller_mode_callback, 10)
        # =================/control_mode 구독 =================
    ...
    def pure_pursuit_control(self):
        ...
        if self.cur_speed_ms < 2.0:  # 7.2 km/h 이하
            self.lfd_gain = 0.3
        elif self.cur_speed_ms < 4.5:  # 18 km/h 이하
            self.lfd_gain = 0.4
        else:  # 20 km/h 이상
            self.lfd_gain = 0.5
        ...
        # 8) 시각화 & 커맨드 퍼블리시
        self.publish_lookahead_visuals(theta, lfd)
    ...
    def publish_lookahead_visuals(self, theta: float, lfd: float):
        # 1) forward point marker
        m = Marker()
        ...
        # 2) lookahead radius marker (원)
        circle_marker = Marker()
        circle_marker.header.frame_id = 'vehicle_frame'
        circle_marker.header.stamp = self.get_clock().now().to_msg()
        circle_marker.ns = 'lookahead_radius'
        circle_marker.id = 1
        circle_marker.type = Marker.CYLINDER  # RViz에서 원 형태
        circle_marker.action = Marker.ADD
        circle_marker.pose.position.x = 0.0  # 차량 중심
        circle_marker.pose.position.y = 0.0
        circle_marker.pose.position.z = 0.0
        circle_marker.pose.orientation.w = 1.0
        circle_marker.scale.x = lfd * 2  # 반지름 * 2
        circle_marker.scale.y = lfd * 2
        circle_marker.scale.z = 0.05  # 두께
        circle_marker.color.a = 0.3
        circle_marker.color.r = 0.9
        circle_marker.color.g = 0.0
        circle_marker.color.b = 0.0
        self.lookahead_marker_pub.publish(circle_marker)
