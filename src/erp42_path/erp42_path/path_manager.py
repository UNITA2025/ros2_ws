...
class PathManager(Node):
    ...
    def __init__(self):
        ...
        self.parking_foward_start_time = None
    ...
    def main_loop(self):
        ...
        if self.current_mode == "normal":
            ...
            elif time.time() -  self.parking_start_time < self.pakring_steer_duration : # 주차 중에는 계속 주차 경로 명령
                self.pakring_steer_cmd()
            elif time.time() -  self.parking_foward_start_time < self.parking_foward_duration :# 주차 후 전진
                cmd = ErpCmdMsg()
                cmd.gear = 0  # 전진
                cmd.speed = 70
                cmd.steer = 0
